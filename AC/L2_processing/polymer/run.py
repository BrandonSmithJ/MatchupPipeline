	...
	**extra_cmd,                            # Any extra commands to pass to polymer
	...
	if not out_file.exists() or overwrite:
		...
		if sensor in ['MSI', 'S2A', 'S2B']:
			filename = list(Path(filename).joinpath('GRANULE').glob('*/'))[0]
		...
		#force_ipv4()
		os.environ['WGETRC'] = Path(__file__).parent.parent.resolve().joinpath('authentication', 'eosdis_wgetrc').as_posix()
		...
		run_atm_corr(
			Level1[sensor](filename, **kwargs),
			Level2_NETCDF(filename=output, overwrite=overwrite),
			**atm_kwargs,
